...
especialidades médicas usando múltiples fuentes:
- Afiliación del autor
- Revistas donde publica
- Términos MeSH de sus artículos
- Palabras clave
- Patrones en títulos/abstracts
...
def get_unique_authors(cur, limit=None, incremental=False):
    ...
    if incremental:
        ...
        query = """
            ...
            LEFT JOIN author_specialties asp ON pa.author_name = asp.author_name
...
def infer_author_specialties(cur, author_name, article_ids):
    ...
    Combina las especialidades de todos los artículos y calcula
    un score agregado para cada especialidad.

    Returns:
        Dict de {snomed_code: {confidence, article_count, sources, name_en, name_es}}
    """
    ...
    for pubmed_id in article_ids:
        try:
            specialties = SpecialtyService.infer_article_specialties(
                cur, pubmed_id, top_n=10, min_confidence=0.1
            )

            for spec in specialties:
                code = spec['snomed_code']
                specialty_data[code]['total_confidence'] += spec['score']
                specialty_data[code]['article_count'] += 1
                specialty_data[code]['sources'].update(spec['sources'])
...
def save_author_specialties(cur, author_name, author_orcid, specialties):
    ...
    for snomed_code, data in specialties.items():
        cur.execute("""
            INSERT INTO author_specialties
                (author_name, author_orcid, snomed_code, confidence, article_count, sources, last_updated)
