...
COMMENT ON TABLE nlm_mesh_terms IS 'Términos MeSH (Medical Subject Headings) de la National Library of Medicine (NLM)';
COMMENT ON COLUMN nlm_mesh_terms.mesh_ui IS 'Identificador único MeSH (ej: D002318)';
COMMENT ON COLUMN nlm_mesh_terms.mesh_name IS 'Nombre principal del término MeSH';
COMMENT ON COLUMN nlm_mesh_terms.tree_numbers IS 'Códigos jerárquicos separados por ; (ej: C14;C14.280.647)';
COMMENT ON COLUMN nlm_mesh_terms.parent_category IS 'Categoría raíz: C=Diseases, F=Psychiatry, etc.';
COMMENT ON COLUMN nlm_mesh_terms.year_introduced IS 'Año en que se introdujo el término en MeSH';
